...
class Formatters {
  ...
  /// Formate une date au format jj/mm/aa (ex: 15/06/26).
  ...
  /// Formate une date au format jj/mm/aaaa pour le proforma ISITEK.
  static String dateProforma(DateTime date) {
    final dd = date.day.toString().padLeft(2, '0');
    final mm = date.month.toString().padLeft(2, '0');
    return '$dd/$mm/${date.year}';
  }

  /// Alias pour le générateur PDF (modèle HTML).
  static String frDate(DateTime date) => dateProforma(date);

  /// Montant sans devise (ex: "30 600 000").
  static String money(double value) => montant(value);

  /// Pourcentage (ex: "15%").
  static String pct(double value) => pourcentage(value);
